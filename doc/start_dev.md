...
[GCC](https://gcc.gnu.org). To use it, assign `COVERAGE=1` at the top of the
root Makefile. Then, `make check` to run all tests. (Consider doing `make clean &&
make all` before to ensure that all built files are up-to-date.)

A visual coverage report can be generated by [gcovr](https://gcovr.com/en/stable/guide.html)
([gcovr repository](https://github.com/gcovr/gcovr)). After installing it, try

    gcovr --html-details gr1c-test-coverage.html

and open gr1c-test-coverage.html in a web browser.

Alternatively, the coverage report for a source file can be obtained from `gcov`
by calling it with the file name. E.g.,
